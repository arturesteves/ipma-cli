    ...
    execute_command() {
        get_locations_ids(this.args).then(function(locations_ids) {
            if(this.args["forecast"]) {
                const forecast = this.args["forecast"];
                make_request_test(request.forecast_location_several_days, util.relevant_data, locations_ids, forecast);
                //make_request(locations_ids);
                //make_request(locations_ids, forecast);
            }
            else if(this.args["forecast_day"]) {
                const forecast_day = this.args["forecast_day"];
                make_request(locations_ids);
                //make_request(locations_ids, forecast_day)
            }
            else if(this.args["today"]) {
                make_request(locations_ids);
                //make_request(locations_ids, 0);
            }
            else if(this.args["tomorrow"]) {
                make_request(locations_ids);
                //make_request(locations_ids, 1);
            }
            else {
                make_request(locations_ids);
                //make_request(locations_ids, this.default.forecast);
            }
...
function make_request_test(request_function, handle_result_function, locations_ids, arg) {
    locations_ids.forEach(function (element) {
        request_function(element)
            .then(function (result) {
                console.log(handle_result_function(result, arg));
            })
            .catch(function (error) {
                log.debug(error);
