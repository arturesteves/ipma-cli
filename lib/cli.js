    ...
    execute_command() {
        get_locations_ids(this.args).then(function (locations_ids) {
            if (this.args["forecast"]) {
                ...
                make_request(request.forecast_location_several_days, util.filter_data_multiple_days, locations_ids, null, {forecast: forecast});
            }
            else if (this.args["forecast_day"]) {
                const forecast_day = (this.args["forecast_day"] > this.default.max_single_day_forecast ||
                    this.args["forecast_day"] < 1) ? this.default.today : this.args["forecast_day"];

                make_request(request.forecast_locations_single_day, util.filter_data_single_day, locations_ids, {forecast_day: forecast_day});
            }
            else if (this.args["today"]) {
                make_request(locations_ids);
                make_request(request.forecast_locations_single_day, util.filter_data_single_day, locations_ids, {forecast_day: 0});
            }
            else if (this.args["tomorrow"]) {
                make_request(locations_ids);
                make_request(request.forecast_locations_single_day, util.filter_data_single_day, locations_ids, {forecast_day: 1});
            }
            else {
                make_request(request.forecast_location_several_days, util.filter_data_multiple_days, locations_ids, this.default.forecast);
            }
...
function make_request(request_function, handle_result_function, locations_ids, request_args, handle_result_args) {
    locations_ids.forEach(function (element) {
        request_function(element, request_args)
            .then(function (result) {
                console.log(handle_result_function(result, handle_result_args));
            })
            .catch(function (error) {
                log.debug(error);
