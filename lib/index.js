...
function relevant_data(result) {
    ...
    // return -> Location | tº min - tº max | probabilidade de precipitacao | weather | wind class | date

    // return the result for today if received indication of that and all of the data for all the 5 days
    let temp = data.tMin + "ºC - " + data.tMax + "ºC";
    return get_global_local_id(data.globalIdLocal) + " | " + temp + " | " + data.probPrecipita + "%" + " | " +
        data.forecastDate;
    //return data;
}
